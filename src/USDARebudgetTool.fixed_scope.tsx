...
const TransferPanel: React.FC<{
  ...
  onClear: () => void;
  transfers: any[];
}> = ({ newBudget, currentSnapshot, onAdd, onClear, transfers }) => {
  const [from, setFrom] = useState("");
  const [to, setTo] = useState("");
  const [mode, setMode] = useState<"budget_total" | "direct_to_dest">(
    "direct_to_dest"
  );
  const [amount, setAmount] = useState("");
  const [preview, setPreview] = useState<any>(null);

  const accounts = newBudget.filter(
    (r) =>
      r.account !== "SUMMARY" &&
      r.account !== (ratesData?.wrs_account_numbers?.indirect_costs || "58960")
  );
  ...
  const onPreview = () => {
    try {
      ...
      const { source_out, direct_to_dest, fa_added, eligible } =
        computeTransferImpact(to, cents, mode);
    ...
    <div className="mb-6 border border-gray-200 rounded p-4 bg-gray-50">
          ...
          <button
            ...
            onClick={() => {
              ...
              try {
                ...
                const fromAfter =
                  toCents(fromRow?.proposed_budget ?? 0) - source_out;
                if (fromAfter < toCents(enc))
                  return alert(`Would breach encumbrance on ${from}`);
      ...
      {preview && (
        <div className="mt-3 p-2 border rounded bg-white text-sm">
          <strong>Preview:</strong> Source out ${fromCents(preview.source_out)}{" "}
          • Direct to dest ${fromCents(preview.direct_to_dest)} • F&A to 58960 $
          {fromCents(preview.fa_added)} • MTDC {preview.eligible ? "Yes" : "No"}
        </div>
      )}
      {transfers.length > 0 && (
        <div className="mt-4">
          ...
          <ul className="text-sm list-disc list-inside">
            {transfers.map((t) => (
              <li key={t.id}>
                {t.from} → {t.to} ${fromCents(t.amount_cents)} [{t.mode}]
              </li>
            ))}
          </ul>
        </div>
...
const ProjectedBudgetTable: React.FC<{
  baseline: any[];
  snapshot: any[];
  project: (b: any[], s: any[]) => any;
}> = ({ baseline, snapshot, project }) => {
  let finalRows: any[] = baseline,
    mapping: any[] = [];
  try {
    ...
    return (
      <div className="p-3 bg-red-50 border border-red-200 rounded text-red-800">
        Error projecting transfers: {e.message}
      </div>
    );
  }
  ...
  const addedFA = mapping.reduce(
    (s: number, m: any) => s + (m.fa_added_to_fanda_cents || 0),
    0
  );
  ...
  return (
    <div>
      <div className="mb-2 text-sm text-gray-700">
        Added F&A from transfers:{" "}
        <span className="font-medium">${fromCents(addedFA)}</span>
      </div>
                ...
                row.account !== "SUMMARY" && (
                  <tr key={idx}>
                    ...
                    <td className="border px-4 py-2 text-right">
                      ${Number(row.current_budget).toLocaleString()}
                    </td>
                    <td className="border px-4 py-2 text-right">
                      ${Number(row.proposed_budget).toLocaleString()}
                    </td>
                    <td className="border px-4 py-2 text-right">
                      $
                      {(
                        Number(row.proposed_budget) - Number(row.current_budget)
                      ).toLocaleString()}
                    </td>
                    <td className="border px-4 py-2 text-center">
                      {row.account ===
                      (ratesData?.wrs_account_numbers?.indirect_costs ||
                        "58960")
                        ? "-"
                        : isMtdcEligible(row.account, ratesData)
                        ? "✓"
                        : "✗"}
                    </td>
                  </tr>
              ...
              {mapping.map((m: any, i: number) => (
                <tr key={i}>
                  ...
                  <td className="border px-2 py-1 text-right">
                    ${fromCents(m.source_out_cents)}
                  </td>
                  <td className="border px-2 py-1 text-right">
                    ${fromCents(m.direct_to_dest_cents)}
                  </td>
                  <td className="border px-2 py-1 text-right">
                    ${fromCents(m.fa_added_to_fanda_cents)}
                  </td>
                  <td className="border px-2 py-1 text-center">
                    {m.dest_mtdc_eligible ? "Yes" : "No"}
                  </td>
...
const USDARebudgetTool = () => {
  ...
  const divByOnePlus = (cents: number, rate: number) =>
    Math.round(cents / (1 + rate));
  ...
  const faRateForDest = (destAcct: string, rates: any) =>
    Number(rates?.fa_rate?.rate ?? 0.276);
  ...
  const baselineSum = (rows: any[]) =>
    sum(
      rows
        .filter((r: any) => r.account !== "SUMMARY")
        .map((r: any) => toCents(r.current_budget))
    );
  ...
  const [transfers, setTransfers] = useState<
    {
      ...
      to: string;
      amount_cents: number;
      mode: "budget_total" | "direct_to_dest";
    }[]
  >([]);
  const [mappingLog, setMappingLog] = useState<any[]>([]);

  // Preview (pure)
  const computeTransferImpact = (
    toAcct: string,
    amountCents: number,
    mode: "budget_total" | "direct_to_dest"
  ) => {
  ...
  const applyOne = (
    ...
    if (fromAcct === fandaAcct || toAcct === fandaAcct)
      throw new Error("Direct transfers to/from F&A are not allowed.");
    ...
    const { source_out, direct_to_dest, fa_added, eligible } =
      computeTransferImpact(toAcct, amountCents, mode);
    const enc = encumbrancesFor(fromAcct, snapshot);
    const fromAfter = toCents(fromRow.proposed_budget) - source_out;
    if (fromAfter < toCents(enc))
      throw new Error(`Transfer would breach encumbrance on ${fromAcct}`);
    if (fromAfter < 0)
      throw new Error(`Transfer would make ${fromAcct} negative`);

    fromRow.proposed_budget =
      Number(fromRow.proposed_budget) - Number(fromCents(source_out));
    toRow.proposed_budget =
      Number(toRow.proposed_budget) + Number(fromCents(direct_to_dest));
    fandaRow.proposed_budget =
      Number(fandaRow.proposed_budget) + Number(fromCents(fa_added));
    rows.forEach((r: any) => {
    ...
    if (before !== after)
      throw new Error("Totals do not reconcile after transfer.");
    return { rows, logRow };
  };
          ...
          {activeTab === "adjust" && (
            <div>
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
                <h4 className="text-sm font-medium text-blue-900 mb-2">
                  Manual Transfers (Fine-Tune)
                </h4>
                <p className="text-blue-800 text-sm">
                  Use this to make surgical reallocations after personnel
                  calculations. F&A is added automatically when the destination
                  is MTDC-eligible and posted to the F&A account.
                </p>
              </div>

              {newBudget.length === 0 ? (
                <p className="text-gray-500">
                  Calculate a New Budget first (Personnel tab), then return
                  here.
                </p>
              ) : (
                <>
                ...
                <div className="border border-gray-300 rounded-lg p-6">
                  ...
                  <button
                    onClick={() => {
                      ...
                      const rows = mappingLog.map((m: any) =>
                        header.map((h: string) => String((m as any)[h] ?? ""))
                      );
                      const csv = [
                        header.join(","),
                        ...rows.map((r) => r.join(",")),
                      ...
                      a.href = url;
                      a.download = "rebudget_mapping.csv";
                      a.click();
