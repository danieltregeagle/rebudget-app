    ...
    const TransferPanel: React.FC<{
      ...
      const accounts = newBudget.filter(
        (r) =>
          r.account !== "SUMMARY" &&
          r.account !==
            (ratesData?.wrs_account_numbers?.indirect_costs || "58960")
      );
        ...
        <div className="mb-6 border border-gray-200 rounded p-4 bg-gray-50">
              ...
              <button
                ...
                onClick={() => {
                  ...
                  try {
                    ...
                    const fromRow = newBudget.find(
                      (r: any) => r.account === from
                    );
                    const { source_out } = computeTransferImpact(
                      to,
                      cents,
                      mode
                    );
                    ...
                    if (fromAfter < 0)
                      return alert(`Would make ${from} negative`);
                    onAdd({
          ...
          {preview && (
            <div className="mt-3 p-2 border rounded bg-white text-sm">
              <strong>Preview:</strong> Source out $
              {fromCents(preview.source_out)} • Direct to dest $
              {fromCents(preview.direct_to_dest)} • F&A to 58960 $
              {fromCents(preview.fa_added)} • MTDC{" "}
                    ...
                    row.account !== "SUMMARY" && (
                      <tr key={idx}>
                        ...
                        <td className="border px-4 py-2 text-right">
                          $
                          {(
                            Number(row.proposed_budget) -
                            Number(row.current_budget)
                          ).toLocaleString()}
                        </td>
